...
lpm_main_loop(__rte_unused void *dummy)
{
    ...
    uint64_t count = 0;
    uint64_t mean = 0;
    uint16_t pkts_in_queue = 0;
    uint16_t pkts_vector[500];
    uint64_t times_vector[500];
    int queue_check = 0;

    //Defining write-to file
#if STATS_FILE
    char filename_values[32];
    char filename_times[32];
    ssize_t written_values, written_times;
    if (sprintf(&filename_values, "values_core%u.txt", lcore_id) < 0)
        RTE_LOG(CRIT, L3FWD, "Error in creating filename");
    if (sprintf(&filename_times, "times_core%u.txt", lcore_id) < 0)
        RTE_LOG(CRIT, L3FWD, "Error in creating filename");
    int fd_values = open(filename_values, O_WRONLY | O_CREAT, 0666);
    int fd_times = open(filename_times , O_WRONLY | O_CREAT, 0666);
    if (fd_values < 0 || fd_times < 0)
        RTE_LOG(CRIT, L3FWD, "Error in opening file");
#endif
	...
	while (!force_quit) {
		...
		for (i = 0; i < n_rx_q; ++i) {
			portid = qconf->rx_queue_list[i].port_id;
			queueid = qconf->rx_queue_list[i].queue_id;
			//pkts_in_queue = rte_eth_rx_queue_count(portid, queueid);
            ...
            count++;
#if STATS_FILE
            pkts_vector[queue_check] = nb_rx;
            times_vector[queue_check++] = cur_tsc;
            if (queue_check == 500) {
                queue_check = 0;
                written_values = 0;
                written_times = 0;
                while (written_values < sizeof(uint16_t) * 500 || written_times < sizeof(uint64_t) * 500) {
                    written_values += write(fd_values, pkts_vector + written_values, sizeof(uint16_t)*500 - written_values);
                    written_times += write(fd_times, times_vector + written_times, sizeof(uint64_t)*500 - written_times);
                }
            }
#endif
			if (nb_rx == 0)
